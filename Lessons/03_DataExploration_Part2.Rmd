...
# Check our date column: is it a date object? no its a factor
...
```{r, fig.height = 3, fig.width = 4}
# histogram
#aes command is aesthetics
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

# changing width of bins in histogram
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# changing number of bins, not part of aes command
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# focuses in on spread of data (define extent of x values)
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
# different value being plotted, gage height mean
ggplot(USGS.flow.data) +
...
#color is outside aes
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean), bins = 50) +
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "purple") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 5) +
  scale_x_continuous(limits = c(0, 10))

#
#color within aes applied to x variables
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#two continous variables scatterplot
ggplot(USGS.flow.data) +
  geom_point(aes(x = discharge.mean, y = gage.height.mean))
#scatterplot time series anaylsis 
ggplot(USGS.flow.data) +
