...
rain<-ggplot(Rainfall_BR_processed,
             aes(x = Date, y = Rainfall))+
  geom_line()+
  geom_smooth(method="lm")
print(rain)

temp<-ggplot(Temp_BR_processed,
...
#BR_rain_ts<-ts(BR_complete$Rainfall,frequency=12,start=c(f_year,f_month))

#order is always ymd
rain.ts<-ts(Rainfall_BR_processed$Rainfall, c(1901,1),frequency=12)
temp.ts<-ts(Temp_BR_processed$Temperature_C, c(1901,1),frequency=12)
...
rain.deccomposed<-stl(rain.ts, s.window = "periodic")
temp.decomposed<-stl(temp.ts, s.window ="periodic")

plot(rain.deccomposed)
plot(temp.decomposed)
...
temp.components<-as.data.frame(rain.deccomposed$time.series[,1:3])
rain.components<-as.data.frame(rain.decomposed$time.series[,1:3])

BR_rain_components<-mutate(rain.components,
                           Observed = BR_complete$Rainfall,
                           Date=BR_complete$Date)
BR_Temp_components<-mutate((temp.components,
