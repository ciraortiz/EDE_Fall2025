...
#Read Eno river data
EnoDischarge <- read.csv("./Data/Processed/Processed_KEY/USGS_Site02085000_Flow_Processed.csv",
...
```{r}
#creating different datasets to filter out gap years
...
```{r}
?stl
#first variable for stl function is an univariate ts, second variable s.window - all other ones are defaults
...
#adding two more columns with observed data with discharge and date
...
#na.approx interpolates na - we can't just remove them with na.omit or drop unless they're at the beginning or end of data set (zoo is name of package- don't have to library it)
...
f_month <- month(first(wind_data_clean$DATE)) #first month is equivalent to the first month observation date
f_year <- year(first(wind_data_clean$DATE))
wind_data_ts <- ts(wind_data_clean$AWND.clean,
                   start=c(f_year,f_month),
                   frequency=12) #monthly data

#decompose w/ stl function 
...
#shows results w/ tau and value of s 
wind_data_trend2 <- trend::smk.test(wind_data_ts)
# Inspect results
wind_data_trend2
summary(wind_data_trend2)

#Visualization - plot wind dat against time with linear regression 
wind_data_plot <-
