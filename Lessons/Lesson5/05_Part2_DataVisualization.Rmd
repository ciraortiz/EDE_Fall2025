...
O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray()  #default ggplot theme ^same as above
print(O3plot1)
...
O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light()  #grey lines/axes on white backgrounds 
print(O3plot3)
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
#defining my own theme
mytheme <- theme_classic(base_size = 14) +  #increasing font size
...
O3plot5 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  mytheme  #setting theme within the plot environment
print(O3plot5)  
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) +  #horizontal line that crosses at 50 
  geom_hline(yintercept = 100, lty = 2) +  #horizontal line that crosses at 100
  geom_point(alpha = 0.5, size = 1.5) +  #alpha adds transparency to points
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") +  #abbreviated months to three letters
  ylab(expression("O"[3]* " AQI Value")) + #specifying y axis label 
