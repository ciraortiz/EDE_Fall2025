...
nutrient_data <- read.csv(
  here('Data/Processed/Processed_KEY','NTL-LTER_Lake_Nutrients_PeterPaul_Processed.csv'),
...
my_plot <- nutrient_data %>% 
  ...
  ggplot(aes(
    ...
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) +  #alpha for transparency
  scale_x_date(
    date_breaks = "1 years",
    date_labels = '%Y') +
  scale_shape_manual(values = c(17, 15)) +  #17 is triangle, 15 is square
  labs(
...
my_plot <- nutrient_data %>% 
  ...
  ggplot(aes(
    x = sampledate,
    y = !!sym(the_parameter),#reading as a variable not column name
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) +  #alpha for transparency
  scale_x_date(
...
#change output and runtime at top to not knit
selectInput(
...
sliderInput(
  ...
  step = 1,
  round = T,
  sep='' #takes commas out of years on slider scale
)
...
renderPlot(
  plot_it(
    input$the_nutrient,
    input$year_slider[1],#start year
    input$year_slider[2]#endyear
  )
)

#adding text to output screen
renderText(
