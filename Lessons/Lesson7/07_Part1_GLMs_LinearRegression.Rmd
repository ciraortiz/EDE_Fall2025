...
PeterPaul.chem.nutrients <- read.csv(here("Data/Processed/Processed_KEY/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv"), stringsAsFactors = TRUE)
...
#transforming screen to grid - par mfrow, mar is size
...
``` {r, fig.height = 3, fig.width = 4}
#subsetting entire data set for only observations for Paul Lake, column total phosphorus and depth and dissolved oxygen
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), 
                   tp_ug ~ depth + dissolvedOxygen)
summary(TPregression)
#r squared is .29 = 30% tpug is due to depth and dissolved oxygen

#visualizing regression with scatter plot
...
library(corrplot)
#creating new data frame with columns of interest and omitting n/as
PeterPaulnutrients <- 
...
#printing the upper diagonal only
...
#filtering for paul lake data only and omit nas
Paul.naomit <- PeterPaul.chem.nutrients %>%
  filter(lakename == "Paul Lake") %>%
  na.omit()
#total phosphorus is a function of 5 different variables
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4)

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC) #shows best regression is with oxygen, temp, and tnug
