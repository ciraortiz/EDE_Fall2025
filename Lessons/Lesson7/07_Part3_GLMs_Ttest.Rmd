...
One sample - continuous response, tests null hypothesis that the mean of the group is equal to a specific value
Two sample - cont response, testing the mean of two samples are equal
...
```{r, message = FALSE}
EPAair <- read.csv(here("Data/Processed/Processed_KEY/EPAair_O3_PM25_NC1819_Processed.csv"), stringsAsFactors = TRUE)
...
O3.onesample
#pvalue is less than .05 so we reject the null hypothesis, so mean is less than 50

Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  geom_density(fill = "gray") +
  geom_vline(xintercept = 50, color = "#238b45", lty = 2, size = 0.9) + #adding vertical line at 50
...
# Format as a GLM - asking year a predictor of ozone levels
