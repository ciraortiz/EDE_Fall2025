...
```{r, results = "hide"}
#gives you introductio to dplyr info in the help tab
vignette("dplyr")
```
...
# matrix filtering - filtering for only rows that have rows with depth column = 0 
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]

# dplyr filtering, same result as above
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0)
#filtering for observartions with depth less than 0.25
...
#excluding the other lakes that are not peter & paul 
...
#just choosing paul and peter lake
...
#selecting data from june through october (different ways)
...
```{r arranging data}
#arranding depth column in ascending order
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth)
#arranding depth column in descending order
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth))

# Exercise: 
# Arrange NTL.phys.data by temperature, in descending order. 
NTL.phys.data.temp<(-NTL.phys.data.temp$temperature_C)
NTL.phys.data.temp.ascending <- arrange(NTL.phys.data, depth)
...
```{r selecting columns}
#selecting columns with lakename, sample date and temp (excluding other columns)
...
```{r mutating data}
#adding new column (farenheight temp) that is function of original temp column (celsius)
...
#another way to find start and end dates of dataset
...
```{r using pipes}
#combining previous operations into one using a pipe - creating a new data frame, using sign operation because creating new, filtering to get paul and peter lake, select temp columns and creating fareneheight column with mutate
...
to separate the raw versus processed data and be able to easily identify what we did to wrangle the data 
