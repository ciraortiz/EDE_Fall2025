...
#Install packages happens once
#library is like opening the app
#tidyverse includes dplyr, ggplot, and other packages like pipes
library(tidyverse)
library(lubridate)
...
#Alternatively, use the tidyverse/dplyr "glimpse" function (same as str)
...
NTL.phys.data199<-filter(NTL.phys.data, year4 %in% c('1999'))

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data9099<-filter(NTL.phys.data, lakename =="Tuesday Lake", year4 %in% c(1990:1999))
# == and %in% function as the same 
# can use , or & when filtering 
```
...
#Add pipes in the correct place below, adding pipe symbol after every distinct function
...
#for July: you could mutate to create a new column for months
#month(sampledate) can be used for year and date only if it is formated a date
NTL.phys.dataex3<-NTL.phys.data %>%
...
irradiance<-pivot_longer(NTL.phys.data,
                        cols = "irradianceWater":"irradianceDeck",
             names_to = "irradianceType",
...
tempc<-pivot_wider(NTL.phys.data,
               names_from="depth",
